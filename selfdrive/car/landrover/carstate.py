...
class CarState(CarStateBase):
  def __init__(self, CP):
    ...
    self.shifter_values = can_define.dv["GEAR_SHIFT"]['GEAR_PRND']

  def update(self, cp, cp_cam):

    ret = car.CarState.new_message()

    self.frame = int(cp.vl["EPS_01"]['COUNTER'])
    ...
    ret.wheelSpeed.fr = v_wheel # cp.vl["SPEED_04"]["WHEEL_SPEED_FR"]
    ret.wheelSpeed.fl = v_wheel # cp.vl["SPEED_04"]["WHEEL_SPEED_FL"]
    ret.wheelSpeed.rr = v_wheel # cp.vl["SPEED_03"]["WHEEL_SPEED_RR"]
    ret.wheelSpeed.rl = v_wheel # cp.vl["SPEED_03"]["WHEEL_SPEED_RL"]
    ret.vEgoRaw = v_wheel
    ...
    ret.gearShifter = self.parse_gear_shifter(self.shifter_values.get(cp.vl['GEAR_SHIFT']['PRND'], None))
    ...
    ret.steeringRateDeg = self.angle_steers_rate
    ...
    ret.steeringTorqueEps = cp.vl["EPS_04"]["STEER_TORQUE_EPS04"]
